// Migration Runner - StudyVault V1.0
// Handles database migrations and versioning
//
// IMPORTANT: This migration system is simplified to use a single clean initial migration (001).
// This ensures that new installations get a complete database structure in one step.
// No compatibility migrations are needed as this is for clean installations.

import { getDatabase } from '../index';
import { executeQueryFirst, executeWrite } from '../utils';
...
export class MigrationRunner {
  ...
  private async ensureSchemaVersionsTable(): Promise<void> {
    try {
      ...
      console.error('Failed to create schema_versions table:', error);
    }
  }
  /**
    ...
    return [
      {
        ...
        up: async () => {
          ...
          // Notes table - Complete with all columns from the start
          await executeWrite(`
          ...
          const triggers = [
            ...
            `CREATE TRIGGER IF NOT EXISTS update_courses_timestamp 
             ...
             BEGIN UPDATE courses SET updated_at = CURRENT_TIMESTAMP WHERE id = NEW.id; END`,
            ...
            `CREATE TRIGGER IF NOT EXISTS update_events_timestamp 
             ...
             BEGIN UPDATE calendar_events SET updated_at = CURRENT_TIMESTAMP WHERE id = NEW.id; END`,
            
            `CREATE TRIGGER IF NOT EXISTS set_task_completed_date
             AFTER UPDATE OF status ON tasks FOR EACH ROW
             WHEN NEW.status = 'completed' AND OLD.status != 'completed'
             BEGIN UPDATE tasks SET completed_date = CURRENT_TIMESTAMP WHERE id = NEW.id; END`,
            
            `CREATE TRIGGER IF NOT EXISTS update_notes_word_count
             AFTER UPDATE OF content ON notes FOR EACH ROW
             BEGIN 
               UPDATE notes SET 
        ...
        down: async () => {
          ...
          for (const table of tables) {
            await executeWrite(`DROP TABLE IF EXISTS ${table}`);
          }
        }
      }
    ];
