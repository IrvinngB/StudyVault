...
interface AddCategoryFormProps {
  classId: string
  porcentajeActual: number // ✅ nueva prop
  onSuccess: (categoria: CategoryGradeData) => void
  onCancel?: () => void
}

export default function AddCategoryForm({ classId, porcentajeActual, onSuccess, onCancel }: AddCategoryFormProps) {
  const { theme } = useTheme()
  const [formData, setFormData] = useState({ name: '', percentage: '' })
  const [error, setError] = useState('')
  const [loading, setLoading] = useState(false)

  const porcentajeMaximoPermitido = Math.max(100 - porcentajeActual, 0)
  ...
  const handleSubmit = async () => {
    ...
    if (porcentajeNum > porcentajeMaximoPermitido) {
      setError(`Solo puedes asignar hasta ${porcentajeMaximoPermitido}% en esta categoría`)
      return
    }
  ...
  return (
    <ThemedCard variant="outlined" padding="large" style={{ marginTop: theme.spacing.md }}>
      ...
      <View style={{ gap: theme.spacing.md }}>
        ...
        <ThemedInput
          label={`Porcentaje que representa (máximo ${porcentajeMaximoPermitido}%)`}
